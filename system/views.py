...
def system_config_view(request):
    ...
    try:
        ...
        if _is_ajax(request):
            return _json_response(500, f"系统配置操作失败: {str(e)}")
        messages.error(request, f"系统配置操作失败: {str(e)}")
...
def system_logs_view(request):
    ...
    try:
        ...
        messages.error(request, f"加载操作日志失败: {str(e)}")
...
def system_backup_view(request):
    ...
    try:
        ...
        if _is_ajax(request):
            return _json_response(500, f"备份操作失败: {str(e)}")
        messages.error(request, f"备份操作失败: {str(e)}")
...
def download_backup_view(request, filename):
    ...
    try:
        ...
        messages.error(request, f"下载备份文件失败: {str(e)}")
        return redirect("system_backup")
...
def delete_backup_view(request, filename):
    ...
    try:
        ...
        _write_operation_log(request, "data_backup", f"删除数据库备份: {filename}")
        ...
        if _is_ajax(request):
            return _json_response(500, f"删除备份文件失败: {str(e)}")
        messages.error(request, f"删除备份文件失败: {str(e)}")
        return redirect("system_backup")